        ...
        for model_name in model_names:
            ...
            if not done and staging_db.exists():
                ...
                if progress:
                    ...
                    status = f"BUILD [0/{len(PHASE_NAMES)}] starting"
...
def generate_makefile(entries: list[dict[str, Any]], output_folder: Path) -> str:
    """Generate a self-contained Makefile encoding the true phase dependency DAG.

    Each permutation gets its own block of sentinel targets. The `all` target
    depends on the `metadata` sentinel of every permutation, so `make all -jN`
    builds all databases with maximum parallelism across both permutations and
    phases.

    The true data DAG (fan-out from chunks):
    ...
    """
    abs_output = str(output_folder.resolve())

    lines: list[str] = [
        "# Generated by: uv run -m benchmarks.demo_builder manifest --makefile",
        "# Usage: make all -j8",
        "#",
        f"OUTPUT_FOLDER := {abs_output}",
        "SENTINEL_DIR  := $(OUTPUT_FOLDER)/_build/sentinels",
        "",
        ".PHONY: all clean",
        "",
    ]

    # Build the `all` target listing metadata sentinels for every permutation.
    perm_ids = [e["permutation_id"] for e in entries]
    metadata_sentinels = " \\\n    ".join(f"$(SENTINEL_DIR)/{p}.metadata" for p in perm_ids)
    lines += [
        f"all: {metadata_sentinels}",
        "",
        # clean: remove sentinels + all per-permutation staging dirs.
        # Use OUTPUT_FOLDER/_build (parent of sentinels/) so a single rm -rf
        # cleans both the sentinel files and the staging DBs in one shot.
        # This is the correct recovery path when a run-phase fails mid-build:
        #   make -f generated.mk clean && make -f generated.mk all -j8
        "clean:",
        "\trm -rf $(OUTPUT_FOLDER)/_build",
        "",
    ]

    # Per-permutation sentinel blocks.
    for entry in entries:
        p = entry["permutation_id"]
        book_id = entry["book_id"]
        model = entry["model_name"]
        s = "$(SENTINEL_DIR)"

        def t(phase: str, s: str = s, p: str = p) -> str:  # noqa: E306
            return f"{s}/{p}.{phase}"

        cmd = f"\tuv run -m benchmarks.demo_builder run-phase --book-id {book_id} --embedding-model {model}"
        ...
        lines += [
            ...
            f"{t('chunks')}: | clean",
            f"{cmd} chunks",
            "\t@mkdir -p $(SENTINEL_DIR) && touch $@",
            "",
            f"{t('chunks_embeddings')}: {t('chunks')}",
            f"{cmd} chunks_embeddings",
            "\t@touch $@",
            "",
            f"{t('chunks_umap')}: {t('chunks_embeddings')}",
            f"{cmd} chunks_umap",
            "\t@touch $@",
            "",
            f"{t('ner')}: {t('chunks')}",
            ...
            "\t@touch $@",
            "",
            f"{t('entity_embeddings')}: {t('ner')}",
            f"{cmd} entity_embeddings",
            "\t@touch $@",
            "",
            f"{t('entities_umap')}: {t('entity_embeddings')}",
            f"{cmd} entities_umap",
            "\t@touch $@",
            "",
            f"{t('entity_resolution')}: {t('entity_embeddings')} {t('relations')}",
            f"{cmd} entity_resolution",
            "\t@touch $@",
            "",
            f"{t('node2vec')}: {t('entity_resolution')}",
            f"{cmd} node2vec",
            "\t@touch $@",
            "",
            # metadata joins all three leaf paths: node2vec + entities_umap + chunks_umap
            f"{t('metadata')}: {t('node2vec')} {t('entities_umap')} {t('chunks_umap')}",
            f"{cmd} metadata",
            "\t@touch $@",
            "",
        ]
