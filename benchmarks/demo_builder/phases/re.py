...
Backends:
  "gliner2" (default) — fastino/gliner2-base-v1, 205M DeBERTa-v3-base.
      ...
      no pre-provided NER spans or spaCy tokenisation required).
...
from benchmarks.demo_builder.constants import GLINER2_RE_LABELS, GLIREL_LABELS
...
# Chunks per RE inference batch call.
# DeBERTa-v3-large (GLiREL) at ~400 subword tokens/chunk: batch=8 ≈ 1.2 GB peak on MPS.
# GLiNER2 (DeBERTa-v3-base) is lighter; same batch size is a safe default.
RE_BATCH_SIZE = 8
...
class PhaseRE(Phase):
    ...
    def __init__(self, labels: list[str] | None = None, backend: str = "gliner2") -> None:
        self._model: Any = None  # GLiNER2 (gliner2 path) or GLiREL (legacy path)
        self._nlp: Any = None  # spaCy Language object (legacy path only)
        self._backend = backend
        if labels is not None:
            self._labels = labels
        elif backend == "gliner2":
            self._labels = GLINER2_RE_LABELS
        else:
            self._labels = GLIREL_LABELS
    ...
    def is_stale(self, conn: sqlite3.Connection) -> bool:
        """Return True if any unprocessed chunks remain.

        GLiNER2: checks ALL chunks (RE discovers entities from raw text).
        Legacy:  checks only chunks that have ≥1 entity (GLiREL needs NER spans).
        """
        try:
            unprocessed: int
    ...
    def _run_gliner2(self, conn: sqlite3.Connection, ctx: PhaseContext) -> None:
        """GLiNER2 RE: process all unlogged chunks — no spaCy, no entity spans."""
        chunks = conn.execute("""
        ...
        # Commit every ~6 batches (~48 chunks) to release the SQLite write lock.
        # Required for parallel builds: entity_embeddings / entities_umap need
        # write-lock windows while relations is running.
        re_commit_batches = 6
        ...
        for batch_num, batch_start in enumerate(range(0, len(chunks), RE_BATCH_SIZE)):
            ...
            # Log ALL chunks in this batch as processed upfront (even those
            # that produce no relations — RE may legitimately find nothing).
            conn.executemany(
                "INSERT OR IGNORE INTO re_chunks_log (chunk_id, processed_at) VALUES (?, ?)",
                [(cid, ts) for cid in chunk_ids],
            )

            # Periodic commit to release the write lock for parallel phases.
    ...
    def _run_legacy(self, conn: sqlite3.Connection, ctx: PhaseContext) -> None:
        """GLiREL RE: process entity-bearing chunks using spaCy tokenisation."""
        nlp = self._nlp
        ...
        for batch_num, batch_start in enumerate(range(0, len(chunks), RE_BATCH_SIZE)):
            ...
            # Log ALL chunks in this batch as processed (even those skipped for
            # having <2 NER spans — RE can't extract anything without 2 entities).
            conn.executemany(
                "INSERT OR IGNORE INTO re_chunks_log (chunk_id, processed_at) VALUES (?, ?)",
                [(cid, ts) for cid in chunk_ids],
            )
            ...
            # Filter to chunks that have ≥2 entity mentions (minimum for RE).
            eligible_mask = [len(entities_by_chunk.get(cid, [])) >= 2 for cid in chunk_ids]
            if not any(eligible_mask):
                continue

            # Batched spaCy tokenisation via nlp.pipe() — faster than per-doc nlp().
            ...
            # Build per-chunk token lists, NER spans, and span→name lookups.
            ...
            # Single batched GLiREL inference call for all eligible chunks.
            ...
            # Map predictions back to insert rows using each chunk's span lookup.
            insert_rows = []
