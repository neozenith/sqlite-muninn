...
def get_embeddings(
    ...
    """Get UMAP embeddings for an HNSW index from its pre-computed {index_name}_umap table.

    All demo databases have pre-computed UMAP tables generated by the demo_builder pipeline.
    Returns 404 if the table is missing (the database was not built with UMAP pre-computation).
    """
    ...
    if not has_umap_table:
        raise HTTPException(
            status_code=404,
            detail=f"No pre-computed UMAP table '{umap_table}' found for index '{index_name}'. "
            "Rebuild the database with the demo_builder pipeline to generate UMAP projections.",
        )

    if dimensions == 2:
        rows = conn.execute(f"SELECT id, x2d AS x, y2d AS y FROM [{umap_table}]").fetchall()
    else:
        rows = conn.execute(f"SELECT id, x3d AS x, y3d AS y, z3d AS z FROM [{umap_table}]").fetchall()
